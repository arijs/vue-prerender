...
function propertySourceHopOnly(key, target, source) {
	if (hop.call(source, key)) {
		target[key] = source[key];
	}
}
function propertySourceNotEmpty(key, target, source) {
	source = source && source[key];
	if (null != source) target[key] = source;
}
function propertyUnfilled(key, target, source) {
	source = source && source[key];
	if (null != source && null == target[key]) {
		target[key] = source;
	}
}
...
extendNewOnly(extend, {
	...
	propertySourceHopOnly: propertySourceHopOnly,
	propertySourceNotEmpty: propertySourceNotEmpty,
	propertyUnfilled: propertyUnfilled,
